   ...
   is
      ...
      begin
         if Is_Application (Item) or Is_Lambda (Item) then
            declare
               Addr : constant Cell_Address :=
                 To_Cell_Address (Item);
            begin
               --  workaround for Aquarius bug
               --  if Addr in From.Cells'Range then
               if Addr >= From.Cells'First
                 and then Addr <= From.Cells'Last
               then
                  X := Car (From, Addr);
                  Y := Cdr (From, Addr);
                  Copy (X);
