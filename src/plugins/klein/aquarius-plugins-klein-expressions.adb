...
package body Aquarius.Plugins.Klein.Expressions is

   --  function Show_Expression (Expr : Program_Tree) return String;
   ...
   is
      Node      : constant Program_Tree := Program_Tree (Target);
   begin
   ...
   is
      Candidates : constant Aquarius.Entries.Array_Of_Entries :=
        Inference.Get_Function_Candidates (Expr.Symbol_Table,
         ...
         begin
            ...
            begin
               if not Aquarius.Entries.Is_Null (Found_Match) then
                  ...
                  Expr.Set_Type
                    (Get_Result_Type
                       (Object_Entry_Type (Found_Match)));
                  Inference.Set_Inferred_Types
                    (Expr,
                     Aquarius.Types.Single_Possible_Type (Expr.Get_Type));
               end if;
            end;
         end;
      end if;
   ...
   is
   begin
      Inference.Set_Possible_Types (Expr, Possibles);
      Inference.Check_Types (Expr);
   end Check_Primary;
   ...
   is
      ...
      Expr      : constant Program_Tree :=
        Program_Tree (Tree.Property (Plugin.Property_Expression));
      Possibles : constant Aquarius.Types.Possible_Types :=
        Inference.Get_Possible_Types (Tree);
   begin
      Check_Expression (Expr, Possibles);
      Inference.Set_Inferred_Types
        (Tree, Inference.Get_Inferred_Types (Expr));
   ...
   begin
      ...
      for I in Inferred_Types'Range loop
         Inferred_Types (I) := Inference.Get_Inferred_Types (Args (I));
      end loop;
   ...
   begin
      ...
      Inference.Set_Inferred_Types (Node,
                                    Inference.Get_Inferred_Types
                                      (Node.Chosen_Tree));
   end Primary_After;
...
--     function Show_Expression (Expr : Program_Tree) return String is
--     begin
--        if Is_Leaf (Expr) then
--           return Expr.Text;
--        else
--           return "(" & Show_Expression (Get_Left (Expr)) &
--             " " & Get_Function_Name (Expr) & " " &
--             Show_Expression (Get_Right (Expr)) & ")";
--        end if;
--     end Show_Expression;
