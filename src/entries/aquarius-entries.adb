package body Aquarius.Entries is
   ...
   is
   begin
      Aquarius.Watchers.Add_Watcher (To_Object.Watchers, W, Context);
   end Add_Watcher;
   ...
   procedure Add_Watcher
     (To_Object : not null access Symbol_Table_Record;
      W         : not null access Watcher'Class;
      Context   : not null access Root_Aquarius_Object'Class)
   is
   begin
   ...
   function Create_Named_Proposition_Constraint
     (Name        : in String;
      Proposition : in Proposition_Evaluator)
     return Entry_Constraint'Class
   is
   ...
   begin
      if New_Name /= Name (Item.all) then
         ...
         Aquarius.Watchers.Signal_Change (Item, Item.Watchers);
         if Item.Owner /= null then
            Aquarius.Watchers.Signal_Change (Item.Owner, Item.Owner.Watchers);
         end if;
      end if;
   end Rename;
   ...
   function Search (Table      : access Symbol_Table_Record;
                    Constraint : in     Entry_Constraint'Class)
                   return Array_Of_Entries
   is
      No_Result : Array_Of_Entries (1 .. 0);
   begin
